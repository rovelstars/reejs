...
class DebuggerButton extends Component {
    render(){
        return html`<button id="ree-debugger"
        ...
        return html`<!-- Main modal -->
          ...
          >
            <div class="relative p-4 w-full max-w-2xl h-full md:h-auto mx-auto">
              <!-- Modal content -->
              <div class="relative bg-white/50 rounded-lg shadow dark:bg-gray-900/70 bg-backdrop-lg backdrop-blur">
                <!-- Modal header -->
                <div
                  class="flex justify-between items-start p-4 rounded-t border-b dark:border-gray-600"
                >
                  <h3 class="text-xl font-semibold text-gray-900 dark:text-white px-4 select-none">
                    Devtools Coming Soon!
                  </h3>
                  <button
                    type="button"
                    class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white"
                    onclick=${()=>{document.getElementById("ree-devtools").classList.add("hidden");}}
                    >
                    <svg
                      class="w-5 h-5"
                      fill="currentColor"
...
render(html`<${DebuggerButton} />`,document.getElementById("ree-debugger-shell"));
