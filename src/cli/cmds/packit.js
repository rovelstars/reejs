...
let apis = fs.readdirSync(apisDir).filter((file) => {
...
export default async function (prog) {
  prog
    ...
    .action(async (service) => {
      if (["node", "bun", "deno-deploy", "workers","vercel"].indexOf(service) === -1) {
        console.log(
      ...
      let ccomponents = await Promise.all(
      ...
      console.log(
        `Transpiled ${pages.length} pages, ${
          components.length
        } components and ${apis.length} apis in ${(end - start) / 1000}s`
      );
      start = Date.now();
      ...
      //copy the package.json, import_map.json, .reecfg, and src folder to the packit folder
      fs.copyFileSync(
