...
export default async function (prog) {
  prog
    ...
    .action(async function (task) {
      ...
      let reecfg = {};
      try {
        reecfg = await fs.readFile(path.join(process.cwd(), "reecfg.json"), "utf-8");
        reecfg = JSON.parse(reecfg);
      } catch (e) { }
      let pkgjson = await fs.readFile(path.join(process.cwd(), "package.json"), "utf-8");
      pkgjson = JSON.parse(pkgjson);

      let tasks = Object.keys(reecfg.tasks || {});
      tasks = tasks.concat(Object.keys(pkgjson.scripts));
      if (!task) {
        console.log("%c[REEJS] %cAvailable tasks: ", "color: #805ad5", "color: green");
        tasks.forEach((task) => {
          console.log(`%c- %c${task} %c${pkgjson.scripts[task] ? "(package.json)" : ""}`, "color: gray", "color: blue", "color: gray; font-style: italic");
          //show the command
      ...
      if (!tasks.includes(task)) {
        console.log("%c[REEJS] %cTask not found!", "color: #805ad5", "color: red");
        return;
      }
      ...
      taskCmds.forEach(async (taskCmd) => {
        ...
        console.log("%c[REEJS] %cTask: %c" + task + " %c" + cmd + " " + args.join(" "), "color: #805ad5", "color: green", "color: blue", "");
