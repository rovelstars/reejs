  ...
  prog
    ...
    .action(async (name, opts) => {
      ...
      if (opts.features) {
        ...
        console.log(
          "%c[REEJS] %cFeatures to be installed: " + opts.features,
          "color: #805ad5",
          "color: green",
        );
      }
      if (!opts.features) {
        console.log(
          "%c[REEJS] %cNo features specified, creating a basic project.",
          "color: #805ad5",
          "color: blue",
        );
      }
      ...
      fs.writeFileSync(
        path.join(process.cwd(), name, "reecfg.json"),
        JSON.stringify(
          {
            name: name,
            version: "0.0.1",
      ...
      if (opts.features.includes("api")) {
        ...
        fs.writeFileSync(
          path.join(process.cwd(), name, "src", "pages", "api", "index.js"),
...
export default install(config);`,
        ...
        fs.writeFileSync(
          path.join(process.cwd(), name, "twind.config.js"),
...
export default defineConfig({
  ...
  darkMode: "class",
        ...
        fs.writeFileSync(
          path.join(process.cwd(), name, "tailwind.config.js"),
          "",
        );
      }
      ...
      if (opts.features.length === 0) {
        ...
        fs.writeFileSync(
          path.join(process.cwd(), name, "src", "index.js"),
      ...
      if (opts.features.includes("tailwind")) {
        ...
        optionalDeps2["@twind/with-react"] = optionalDeps["@twind/with-react"];
      ...
      if (
        opts.features.includes("react") ||
        opts.features.includes("static") ||
        opts.features.includes("api")
      ) {
        ...
        if (opts.features.includes("cloudflare"))
          optionalDeps["hono/cloudflare"] =
            "https://esm.sh/hono@3.4.3/cloudflare";
