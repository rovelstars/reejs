...
let pkginfo = DynamicImport(await import("../version.js"));
  ...
  prog
    ...
    .action(async (name, opts) => {
      ...
      const dependencies = {
        ...
        "@reejs/server": pkginfo.server.version,
        "@reejs/utils": pkginfo.utils.version,
        "@reejs/react": pkginfo.react.version,
        "@reejs/imports": pkginfo.imports.version,
      };
      ...
      async function getFeatures() {
          ...
          options: [
            {
              label: "React",
              value: "react",
              hint: "Suggested if preact can't fullfill your needs",
            },
            {
              label: "Preact (compat)",
              value: "preact",
              hint: "Recommended for simple projects",
            },
            {
              label: "Twind CSS",
              value: "twind",
            ...
            },
            {
              label: "Tailwind CSS",
              value: "tailwind",
              hint: "Recommended when you need tailwind plugins",
            },
        ...
        function validate(value) {
          if (value.includes("twind") && value.includes("tailwind"))
            return `You cannot use both twind and tailwind!`;
          //one cannot use both react and preact
          if (value.includes("react") && value.includes("preact"))
            return `You cannot use both react and preact!`;
          //you cannot use million.js without react, and not with preact
          if (value.includes("million") && !value.includes("react"))
            return `You cannot use million.js without react!`;
          if (value.includes("million") && value.includes("preact"))
            return `You cannot use million.js with preact!`;
        }
        if (validate(value)) {
...
export default ${features.includes("tailwind") || features.includes("twind")
      ...
      fs.writeFileSync(
        ...
        ["node_modules", "dist", ".reejs", "packit.build.js"].join("\n"),
        "utf-8"
      );
      ...
      if (features.includes("twind")) {
        ...
        browserimportmap["@twind/with-react"] = importmap["@twind/with-react"];
...
darkMode: "class",
      ...
      if (features.includes("million")) {
        ...
        browserimportmap["million/react"] = GetPackage("million/react", {
          bundle: true,
          isReactPackage: true,
        });
        fs.writeFileSync(
          path.join(dir, "packit.config.js"),
      ...
      if (shouldInstall) {
        ...
        if (!isAvailable) {
          ...
          try {
            ...
            isAvailable = await $`node ${path.join(
        ...
        if (isAvailable == 1) {
          ...
          clack.log.warn(
            g(
              "Reejs is not available globally or locally. Please install it manually"
            )
          );
          clack.note(
            `cd into ${pc.green(projectName)} and run \`${g("reejs install")}\``
          );
        }
        s.stop();
      } else {
