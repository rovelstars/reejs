...
let projectCacheCount =
  fs.existsSync(path.join(processCwd, ".reejs", "cache")) &&
  fs.readdirSync(path.join(processCwd, ".reejs", "cache")).length;
let reejsCacheCount =
  fs.existsSync(path.join(dir, "cache")) &&
  fs.readdirSync(path.join(dir, "cache")).length;
...
export let doctorReport = async function () {
  ...
  if (globalThis?.process?.versions?.node)
    console.log(
      "%c[NODE_VER] %c" + globalThis?.process?.versions?.node,
      "color: #7237C1",
      "color: green"
    );
  if (globalThis?.Deno?.version?.deno)
    console.log(
      "%c[DENO_VER] %c" + globalThis?.Deno?.version?.deno,
      "color: #7237C1",
      "color: green"
    );
  if (globalThis?.Bun?.version)
    console.log(
      "%c[BUN_VER] %c" + globalThis?.Bun?.version,
      "color: #7237C1",
      "color: green"
    );
  console.log(
    "%c[REEJS_CACHE] %c" +
      `${reejsCacheCount} files in main, ${projectCacheCount || "no"} files in project`,
    "color: #7237C1",
    "color: green"
  );
  console.log(
    "%c[REEJS_DIR] %c" + reejsDir + " %c, %c" + path.join(processCwd, ".reejs"),
    "color: #7237C1",
    "color: green",
    "color: gray",
    "color: green"
  ...
  console.log(
    "%c[REEJS_DIR_EXISTS] %c" + fs.existsSync(path.join(processCwd, ".reejs")),
    "color: #7237C1",
    "color: blue"
  );
