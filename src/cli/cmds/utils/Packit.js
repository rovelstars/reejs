...
export let writers = [
  {
    ...
    run: async (helpers, service) => {
      ...
      ${config.disablePreactCompat ? `import {renderToString as render } from "${await getPackage("react-dom/server")}";` :
    ...
    run: async (helpers, service) => {
      ...
      let routeData =
        `export default [${pages.map(({ route, page, savedAt }) => {
          ...
          return `{path: "/${route}", lazy: async() => {let h=await import("./${savedAt.split(".reejs/serve/")[1]}");return {Component:h.default,whole:h}}}`;
        }).join()
        }]`;
      //save file
...
export default async function reender(page, browserFn) {
  ...
  ${(importmap.imports["react-router-dom"] && (config.disableReactRouter ? "false" : "true")) ? `
    ...
    ${config.disablePreactCompat ? 'hydrateRoot($("#root"), React.createElement(RouterProvider, { router: Router }));' :
    ...
    Router.subscribe(d => {
      ...
      let data = choosen?.generateMetadata?.({ req }) || choosen.metadata;
            ${config.disablePreactCompat ? 'hydrateRoot($("head"), React.createElement(Head, { data }));' :
      ...
      if (service == "deno-deploy" || service == "node") {
        reejsSavedFilesString =
          reejsSavedFilesCache
            .map((file) => {
              return `let cache_${file.replace(".reejs/cache/", "").replaceAll(":", "").replaceAll("-", "").replaceAll(".", "").replaceAll("/", "")} = ${service == "node" ? "fs.readFileSync" : "await Deno.readFile"}("${file}");server.app.get("/__reejs/cache/${file.replace(".reejs/cache/", "")}", (c)=>{c.header('Content-type','${contentType(file.replaceAll("/", ""))
          ...
          reejsSavedFilesServe
            .map((file) => {
              ...
              return `let serve_${file.replace(".reejs/serve/", "").replace("loaders/", "").replaceAll(":", "").replaceAll("-", "").replaceAll(".", "").replaceAll("/", "").replaceAll("-", "_")} = ${service == "node" ? "fs.readFileSync" : "await Deno.readFile"}("${file}");server.app.get("/__reejs/serve/${file.replace(".reejs/serve/", "")}", (c)=>{c.header('Content-type','${contentType(file.replaceAll("/", ""))
