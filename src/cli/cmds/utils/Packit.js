...
export let writers = [
  {
    ...
    run: async (helpers, service) => {
      ...
      mainFile = `${process.versions.webcontainer
...
`;
  ...
  },
  {
    ...
    run: async (helpers, service) => {
      let { DATA, mainFile, glob, pages } = helpers;
      let { contentType } = await Import("https://esm.sh/v132/mime-types@2.1.35", {
        internalDir: true,
      });
          ...
          reejsSavedFilesCache
            .map(file => {
              return `let cache_${file
                ...
                .replaceAll("-", "")
                .replaceAll(".", "")
                .replaceAll("/", "")} = ${(service == "node" || service == "bun") ? "fs.readFileSync" : "await Deno.readFile"
          ...
          reejsSavedFilesServe
            .map(file => {
              ...
              return `let serve_${file
                ...
                .replaceAll(".", "")
                .replaceAll("/", "")
                .replaceAll("-", "_")} = ${(service == "node" || service == "bun") ? "fs.readFileSync" : "await Deno.readFile"
      ...
      mainFile +=
        (service != "node" && service != "workers" && service != "deno" && service != "bun")
...
${(service == "workers" || service == "bun") ? 'console.log(`%c  ➜  %c 🚆 : %chttp://localhost:${process.env.PORT || 3000}`,"color: #db2777","color: #6b7280","color: blue; font-weight: bold;");\nexport default server.app;' : ""}
${service == "deno-deploy"
