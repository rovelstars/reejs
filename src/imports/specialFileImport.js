...
lexer = {
  ...
  if (!code) {
    ...
    if (file.split("/")[file.split("/").length - 2] == "node_modules")
      return file;
    //if file is a folder
    if (fs.statSync(file).isDirectory()) {
      if (fs.existsSync(path.join(file, "package.json"))) {
        let pkg = JSON.parse(
          fs.readFileSync(path.join(file, "package.json")).toString()
        );
        if (pkg.main) {
      ...
      //check if the folder has index.js, else throw an error
        ...
        throw new Error(`Folder ${file} doesn't have an index.js file.`);
      else {
...
jsxFragmentPragma : "Fragment",*/
  ...
  //add custom import.meta.url if the code includes it already and we are on nodejs runtime
  if (result.includes("import.meta.url") && service == "node") {
    result = `import.meta.url = "file://${path.join(processCwd, file)
      }";\n` + result;
  }
