//I consider special files as the files that ends with .jsx, .ts and .tsx and maybe others.
...
if (!fs.existsSync(path.join(reejsDir, "serve"))) {
  fs.mkdirSync(path.join(reejsDir, "serve"));
}
let importmap = DynamicImport(
  await import(`${process.env.PWD}/import_map.json`, {
    assert: { type: "json" },
  })
);
let cachemap = DynamicImport(
  await import(`${process.env.PWD}/.reejs/cache/cache.json`, {
    assert: { type: "json" },
  })
);
