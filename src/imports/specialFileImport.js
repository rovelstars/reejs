  ...
  let files = await Promise.all(packs.map(async (pack) => {
    ...
    if (resolvedPluginInfo?.resolved) {
      if (globalThis?.process?.env?.DEBUG || globalThis?.Deno?.env?.get("DEBUG"))
        console.log(`%c[DEBUG] %c${pack.n} %cwas resolved by %c${resolvedPluginInfo.name} %cto %c${resolvedPluginInfo.resolved}`,
          "color: yellow", "color: blue", "color: gray", "color: blue", "color: gray", "color: blue");

      //since the plugin resolved the id, we need to load the file from the plugin
      let loader = globalThis.PACKIT_LOADERS.filter((e) => e.name == resolvedPluginInfo.name)[0];
      ...
      if (outputOfLoader) {
        ...
        fs.writeFileSync(loaderFile + ".js", outputOfLoader.code || outputOfLoader);
        return `../serve/loaders/${path.join(loader.name, URLToFile(pack.n, true))}.js`;
      }
    }
    if (pack.n == "vite" && file.endsWith("packit.config.js")) {
      ...
      if (!fs.existsSync(path.join(".reejs", "packit", "vite", "index.js"))) {
        fs.mkdirSync(path.join(".reejs", "packit", "vite"), { recursive: true });
        fs.writeFileSync(path.join(".reejs", "packit", "vite", "index.js"),
          `import * as cheerio from "${"../.." + (await dl("https://esm.sh/cheerio@1.0.0-rc.12/lib/slim", true)).split(".reejs")[1]}";`+
          fs.readFileSync(path.dirname(import.meta.url).replace("file://", "") + "/vite.js").toString());
      }
