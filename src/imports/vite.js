...
export function defineConfig(config) {
  ...
  config.writers.push({
    ...
    run: async (helpers, service) => {
      ...
      const html = helpers.fs.readFileSync(helpers.path.join(helpers.processCwd, "index.html"), "utf-8");
      const $ = cheerio.load(html);
      //select all script tags with src and type=module
      const scripts = $("script").filter((i, el) => el.attribs.src && el.attribs.type === "module").map((i, el) => el.attribs.src).get();
      ...
      $("head").append(`<script type="importmap">{"imports":${JSON.stringify(importmap.browserImports)}}</script>`);
...
`
      ...
      if (service == "deno-deploy" || service == "node") {
        reejsSavedFilesString =
          reejsSavedFilesCache
            .map((file) => {
              return `let cache_${file.replace(".reejs/cache/", "").replaceAll(":", "").replaceAll("-", "").replaceAll(".", "").replaceAll("/", "")} = ${service == "node" ? "fs.readFileSync" : "await Deno.readFile"}("${file}");app.get("/__reejs/cache/${file.replace(".reejs/cache/", "")}", (c)=>{c.header('Content-type','${contentType(file.replaceAll("/", ""))
                }');return c.body(cache_${file.replace(".reejs/cache/", "").replaceAll(":", "").replaceAll("-", "").replaceAll(".", "").replaceAll("/", "")})});`;
            })
            .join("\n") +
          reejsSavedFilesServe
            .map((file) => {
              if (!file.replace(".reejs/serve/", "").replace("loaders/", "").includes(".")) return "";
              return `let serve_${file.replace(".reejs/serve/", "").replace("loaders/", "").replaceAll(":", "").replaceAll("-", "").replaceAll(".", "").replaceAll("/", "").replaceAll("-", "_")} = ${service == "node" ? "fs.readFileSync" : "await Deno.readFile"}("${file}");app.get("/__reejs/serve/${file.replace(".reejs/serve/", "")}", (c)=>{c.header('Content-type','${contentType(file.replaceAll("/", ""))
                }');return c.body(serve_${file.replace(".reejs/serve/", "").replace("loaders/", "").replaceAll(":", "").replaceAll("-", "").replaceAll(".", "").replaceAll("/", "")})});`;
            })
            .join("\n") +
          publicSavedFiles.map((file) => {
            return `let ${file.replaceAll("/", "__").replaceAll(".", "").replaceAll("-", "_")} = ${service == "node" ? "fs.readFileSync" : "await Deno.readFile"}("${file}");app.get("${file.slice(file.indexOf("/", 2))}",(c)=>{c.header('Content-type','${contentType(file.replaceAll("/", ""))
  ...
  if (process.argv.includes("-d") || process.argv.includes("--dev")) {
    console.clear();
    console.log("");
    console.log(`%c  VITE %cready in %c${Date.now() - (globalThis?.VITE_PLUGIN_STARTED || (Date.now() - 1))} %cms`,
      "color: green; font-weight: bold;", "color: gray;", "color: white; font-weight: bold;", "color: gray;");
    console.log("");
    console.log(`%c  ➜  %cLocal:%c http://localhost:%c${process.env.PORT || 3000}`, "color: green", "font-weight: bold", "color: blue", "color: blue; font-weight: bold");
    console.log(`%c  ➜  %cNetwork:%c use %c--host %cto expose`, "color: green", "font-weight: bold; color: gray", "color: gray", "font-weight: bold", "color: gray");
    console.log("%c  ➜  %cpress%c h %cto show help", "color: green",
      "color: gray", "color: white", "color: gray");
  } else {
    console.log("%cvite %cbuilding for production...", "color: blue", "color: green");
  }
  return config;
}
