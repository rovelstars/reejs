...
export default async function getJSR_URL(_name) {
  ...
  while (data == "") {
    try {
      ...
      console.trace(`Failed to fetch: https://jsr.io/${_pack}/meta.json [${_c++} tries]`);
      if (_c > 5) throw new Error("Failed to fetch the meta.json file after 5 tries. Maybe the package does not exist?");
    }
  }
  let [maxSatisfying, parse, parseRange, format] = (await Promise.all([
    Import("https://deno.land/std@0.217.0/semver/max_satisfying.ts", { internalDir: true }),
    Import("https://deno.land/std@0.217.0/semver/parse.ts", { internalDir: true }),
    Import("https://deno.land/std@0.217.0/semver/parse_range.ts", { internalDir: true }),
    Import("https://deno.land/std@0.217.0/semver/format.ts", { internalDir: true })
  ...
  while (data == "") {
    try {
      data = await (await fetch(`https://jsr.io/${_pack}/${found}_meta.json`)).json();
    } catch (e) {
      console.trace(`Failed to fetch: https://jsr.io/${_pack}/${found}_meta.json [${_c++} tries]`);
      if (_c > 5) throw new Error("Failed to fetch the meta.json file after 5 tries");
    }
  }
