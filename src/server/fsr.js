...
let apis = fs.readdirSync(path.join(pagesDir, "api")).filter(file => {
  return file.endsWith(".js");
});
...
export default async function ReeFSR({ app, opts }) {
  ...
  if (!render) {
    throw new Error(
      "render function must be provided in option when initializing ReeServer",
    );
  }
  for (let page of pages) {
    ...
    let routePath = "/" + route;
    let routeData = DynamicImport(
      await import(await SpecialFileImport(path.join(pagesDir, page))),
    );
